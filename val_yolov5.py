...
def run(
    ...
    if swarm_parameters == None:
        ...
        x = torch.tensor([[ 26.48807,  28.82849,   0.00000,  46.86107],
        [  0.00000, 700.00000,   0.00000,  29.26048],
        [694.23071,  30.93730,   0.00000,   2.31958],
        [684.49310, 700.00000,  44.87767, 659.15698]], device='cuda:0')
        # load patch
        ############################ 
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            preds, train_out = model(im) if training else (model(im, augment=augment), None)
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default="../checkpoints/yolov5l_finetune.pt", help='model path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default="../v0/checkpoints/finetune_yolov5s_onINRIA.pt", help='model path(s)')
    # parser.add_argument('--weights', nargs='+', type=str, default="/home/ps/Baraa/physics_based_image_generation_model_matlab_script/HALOAttack/v0/runs/train/exp2/weights/best.pt", help='model path(s)')
